...
def find_contributor(p2S: List[Tuple[str, int]], c2S: List[Dict[str, int]], role: int, mentor: bool,
					 start: int):
...
def find_contributors_yield(p2S: List[Tuple[str, int]], c2S: List[Dict[str, int]], role: int, mentor: bool,
							start=0):
	for i in range(start, len(c2S)):
		if mentor and c2S[i][p2S[role][0]] >= p2S[role][1] - 1:
			yield i
